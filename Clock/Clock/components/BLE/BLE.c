...
uint16_t connection_handles[2] = {0, 0};
...
void print_uuids() {
    ...
    // Sensor Service UUID
    for(int i=0;i<16;i++){
        printf("0x%02X ", SERVICE_UUID[15-i]);
    }
    printf("\n");
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    switch (event) {
        ...
        case ESP_GATTS_WRITE_EVT: {
            ...
            // Store Data
            if(param->write.len == sizeof(storedData)/sizeof(storedData[0])){
                for (int i = 0; i < param->write.len; i++) {
                    storedData[i] = param->write.value[i];
                }
            }
            ...
            if(sameAddress && num_conns == 2){
                ESP_LOGI(TAG, "Sending Data to Phone\n");
        ...
        case ESP_GATTS_CONNECT_EVT: {
            ...
            for (int i = 0; i < 2; i++) {
                if (connection_ids[i] == 0xFFFF) {
                    ...
                    ESP_LOGI(TAG, "Stored MAC address: %02X:%02X:%02X:%02X:%02X:%02X", 
