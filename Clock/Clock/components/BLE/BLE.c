...
static esp_ble_adv_params_t advertisingParameters = {
    .adv_int_min         = 0x80, // 32 * 0.625 ms = 20 ms
    .adv_int_max         = 0xA0, // 64 * 0.625 ms = 40 ms
...
static esp_ble_conn_update_params_t conn_params = {
    .min_int = 0x10,      // Minimum connection interval (in 1.25ms units)
    .max_int = 0x30,      // Maximum connection interval (in 1.25ms units)
    .latency = 0,           // Connection latency
    .timeout = 0x7D0          // Supervision timeout (in 10ms units)
};
...
static void gatts_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    switch (event) {
        ...
        case ESP_GATTS_CREATE_EVT: {
            ...
            // Start the service
            ESP_LOGI(TAG, "Starting GATT services, handle %d", service_handle);
            ESP_ERROR_CHECK(esp_ble_gatts_start_service(service_handle));
            ESP_LOGI(TAG, "Started GATT services, handle %d", service_handle);
            ...
            ESP_LOGI(TAG, "Adding characteristic to GATT services, handle %d, characteristic %d", service_handle,CHAR_UUID);
            ESP_ERROR_CHECK(
                esp_ble_gatts_add_char(
                    service_handle, &char_uuid,
                    ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                    ESP_GATT_CHAR_PROP_BIT_READ | ESP_GATT_CHAR_PROP_BIT_WRITE | ESP_GATT_CHAR_PROP_BIT_NOTIFY,
                    NULL, NULL
                )
            );
            ESP_LOGI(TAG, "Added characteristic to GATT services, handle %d, characteristic %d", service_handle,CHAR_UUID);
            break;
        }
        ...
        case ESP_GATTS_CONNECT_EVT: {
            ...
            for (int i = 0; i < 2; i++) {
                if (connection_ids[i] == 0xFFFF) {
                    ESP_LOGI(TAG, "Added Connected Device, conn_id %d", conn_id);
            ...
            for (int i = 0; i < 2; i++) {
                if (connection_ids[i] != 0xFFFF){
                    ESP_LOGI(TAG, "Incrementing Number of Device Connected, num_conns %d", num_conns);
                    num_conns++;
                }
            }
            if (num_conns < 2) {
                ESP_LOGI(TAG, "Advertising BLE");
                ESP_ERROR_CHECK(esp_ble_gap_start_advertising(&advertisingParameters));
            }
            
            // After connection, update connection parameters if needed
            ESP_LOGI(TAG, "Updating Connection Parameters BLE");
        ...
        case ESP_GATTS_DISCONNECT_EVT: {
            ...
            for (int i = 0; i < 2; i++) {
                if (connection_ids[i] == conn_id) {
                    ESP_LOGI(TAG, "Removed Connected Device, conn_id %d", conn_id);
...
static void GAPEventHandler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param){
    switch (event){
        case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT: 
            // Wait for Advertising Data to be set
...
void initializeNVS(){
    ...
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ESP_LOGI(TAG, "Failed to Initialize NVS, error =  %d",ret);
        ESP_LOGI(TAG, "Erase NVS");
        ESP_ERROR_CHECK(nvs_flash_erase());

        ESP_LOGI(TAG, "Initialize NVS");
        ret = nvs_flash_init();
        ESP_LOGI(TAG, "Initialize NVS status =  %d",ret);
    }
    ESP_ERROR_CHECK(ret);
    ...
    ESP_LOGI(TAG, "Limit BLE Memory to BLE");
